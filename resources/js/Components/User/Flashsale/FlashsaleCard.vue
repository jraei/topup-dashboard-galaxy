...
<template>
    <div class="flashsale-card group">
        <!-- User Limit Badge -->
        <div v-if="flashItem.batas_user" class="absolute z-10 top-2 right-2">
            <div
                class="px-2 py-1 text-xs text-white bg-black border rounded-full bg-opacity-70 border-primary"
            >
        ...
        <!-- Card Body -->
        <div
            class="relative p-4 bg-dark-lighter rounded-t-lg h-[80%] overflow-hidden group-hover:bg-opacity-90 transition-all duration-300"
        >
            <!-- Game-themed doodle overlay -->
            <div
                class="absolute top-0 right-0 w-full h-full pointer-events-none opacity-15"
            >
                <img
                    v-if="thumbnailImage"
            ...
            <div class="relative z-10">
                <!-- Layanan name -->
                <h3
                    class="text-primary font-bold mb-1 group-hover:text-glow group-hover:scale-[1.02] transition-all duration-300"
                >
                    {{ layanan.nama_layanan }}
                </h3>

                <!-- Produk name -->
                <p class="mb-3 text-sm text-gray-300">{{ produk.nama }}</p>

                <!-- Price comparison -->
                <div class="flex flex-col mb-1 space-x-1">
                    ...
                    <div>
                        ...
                        <span
                            ...
                            class="text-xs bg-primary text-white px-1.5 py-0.5 rounded"
                        >
                            -{{ discountPercentage }}%
                        </span>
                    </div>
        ...
        <!-- Card Footer -->
        <div class="p-3 border-t border-gray-800 rounded-b-lg bg-dark-card">
            <!-- Progress Bar -->
            <div class="relative h-4 mb-1 overflow-hidden bg-gray-800 rounded">
                <div
