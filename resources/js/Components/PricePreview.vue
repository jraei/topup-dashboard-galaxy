    ...
    try {
        ...
        if (response.data.success) {
            ...
            if (response.data.product && response.data.product.thumbnail) {
                thumbnailCache.value[response.data.product.id] =
...
const getMinMaxPrices = (rolePrices) => {
    ...
    const min = prices.reduce((prev, curr) =>
        prev.price < curr.price ? prev : curr
    );
    const max = prices.reduce((prev, curr) =>
        prev.price > curr.price ? prev : curr
    );
...
<template>
    <div
        class="w-full mt-8 overflow-hidden border rounded-lg shadow-lg border-primary/40 bg-dark-card"
    >
        <div
            class="relative px-5 py-4 border-b bg-gradient-to-r from-dark-sidebar to-dark-sidebar/70 border-primary/30"
        >
            <h2 class="text-xl font-bold text-white">Product Price Preview</h2>
            <p class="mt-1 text-sm text-gray-300">
                Select a product to view pricing across all user roles
            </p>
            <div
                class="absolute top-0 right-0 w-64 h-64 rounded-full bg-primary/5 blur-3xl -z-10"
            ></div>
        </div>
        ...
        <div class="p-4 sm:p-6">
            ...
            <div class="grid grid-cols-1 gap-6 md:grid-cols-3">
                <div
                    class="p-5 border rounded-xl bg-dark-sidebar/50 border-primary/30"
                >
                    <label
                        for="product_selector"
                    ...
                    <div
                        v-if="selectedProductId && previewData"
                        class="flex flex-col items-center justify-center mt-8"
                    >
                        <div class="relative group">
                            <div
                                class="absolute inset-0 transition-all duration-300 rounded-full bg-gradient-to-r from-primary/40 to-secondary/40 blur-md -z-10 group-hover:blur-lg"
                            ></div>
                            <img
                                :src="previewData.product.thumbnail"
                                :alt="previewData.product.name"
                                class="object-cover w-32 h-32 mx-auto transition-all duration-300 border-2 rounded-full shadow-lg border-primary/40 shadow-primary/20 group-hover:scale-105"
                                loading="lazy"
                            />
                        </div>
                    ...
                    <div
                        v-else-if="loading"
                        class="flex flex-col items-center justify-center py-10 mt-4"
                    >
                        <div
                ...
                <div class="md:col-span-2">
                    <div
                        v-if="error"
                        class="p-4 text-sm text-white border rounded-lg bg-red-500/30 border-red-500/40"
                    >
                        {{ error }}
                    </div>
                    ...
                    >
                        ...
                        <p class="mt-2 text-sm text-gray-400">
                            Choose a product from the dropdown to view pricing
                        </p>
                    </div>

                    <div
                        v-else-if="
                            previewData && previewData.pricing.length > 0
                        "
                        class="space-y-6 overflow-hidden"
                    >
                        <div
                            v-for="service in previewData.pricing"
                            :key="service.id"
                        ...
                        >
                            <div class="flex items-center justify-between mb-3">
                                ...
                                <div
                                    class="px-3 py-1 text-sm rounded-lg bg-dark-card"
                                >
                            ...
                            <div
                                class="grid grid-cols-1 gap-3 mt-4 sm:grid-cols-2 md:grid-cols-3"
                            >
                                <template
                                    v-for="(
                                        rolePrice, index
                                ...
                                >
                                    <div
                                        :class="[
                                            ...
                                            getMinMaxPrices(service.role_prices)
                                                .min?.roleId ===
                                            rolePrice.role_id
                                    ...
                                    >
                                        ...
                                        >
                                            <span
                                                ...
                                                >{{ rolePrice.role_name }}</span
                                            >

                                            <div
                                                v-if="rolePrice.profit_type"
                                                :class="[
                                                    'px-2 py-0.5 text-xs rounded-full',
                                                    rolePrice.profit_type ===
                                                    'percent'
                                            ...
                                            >
                                                {{
                                                    rolePrice.profit_type ===
                                                    "percent"
                                                        ? `+${rolePrice.profit_value}%`
                                                        : `${rolePrice.profit_value}x`
                                                }}
                                            </div>
                                        </div>
                    ...
                    >
                        ...
                        <p class="mt-2 text-sm text-gray-400">
                            This product has no active services
                        </p>
                    </div>
                </div>
            </div>
