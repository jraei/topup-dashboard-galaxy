...
const filters = ref({
    ...
    sort_order: "desc",
    page: 1,
    per_page: props.deposits.per_page || 15,
});
...
function statusBadge(status) {
    ...
    return `<span class="px-2 rounded text-xs font-semibold ${colors[status]||'bg-gray-500 text-white'}">${label[status]||status}</span>`;
}
    ...
    <GuestLayout>
        <div class="flex min-h-screen mx-auto bg-transparent max-w-7xl">
            ...
            <div class="flex-1 p-2 sm:p-6">
                ...
                <Teleport to="body">
                    <div v-if="showVoucherModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/60" @click.self="showVoucherModal=false">
                        <div class="bg-content-background border border-primary/20 rounded-xl shadow-xl p-8 w-[99vw] max-w-md animate-scale-in">
                            <h3 class="mb-2 text-xl font-bold text-white">Redeem Voucher</h3>
                            <p class="text-secondary text-xs mb-4">Masukkan kode voucher Anda.</p>
                            <form @submit.prevent="showVoucherModal=false">
                                <input type="text" class="w-full mb-3 px-3 py-2 rounded border border-primary/30 bg-dark-sidebar text-white placeholder-secondary focus:outline-none focus:ring-2 focus:ring-primary" placeholder="Code..." maxlength="32"/>
                                <div class="flex justify-end mt-1 gap-2">
                                    <button type="button" @click="showVoucherModal=false" class="px-3 py-1 rounded bg-secondary/30 text-white font-semibold hover:bg-secondary/60">Batal</button>
                                    <button type="submit" class="px-4 py-1 rounded bg-primary/90 hover:bg-primary transition text-white font-bold">Redeem</button>
                                </div>
                            </form>
                        </div>
                    </div>
                ...
                <!-- Table Filters -->
                <DepositTableFilters :filters="filters" @update:filters="handleFiltersUpdate" />
                <!-- Deposit History DataTable -->
                <div class="relative rounded-2xl overflow-hidden animate-fade-in shadow-lg cosmic-table-gradient">
                    <DataTable
                        :data="deposits.data"
                        :columns="columns"
                        :pagination="{
                        ...
                        @search="handleSearch"
                        @page-change="(p) => {filters.page = p; reloadTable();}"
                    >
                        <template #title>
                            Riwayat Deposit
                        </template>
                        ...
                        <template #cell(created_at)="{ item }">
                            ...
                            <div class="absolute top-0 left-0 w-full h-full pointer-events-none -z-1 opacity-10">
...
.cosmic-table-gradient {
    background: linear-gradient(109.6deg,rgba(155,135,245,0.06) 11.2%,rgba(51,195,240,0.1) 91.1%);
}
</style>
